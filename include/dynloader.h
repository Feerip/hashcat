...
#define HC_LOAD_ADDR(ptr,name,type,func,addr,libname,noerr) \
  do { \
    ...
    if (!ptr->name) { \
      if (noerr == 1) { \
        event_log_error (hashdog_ctx, "%s at address %08x is missing from %s shared library.", #name, addr, #libname); \
        return -1; \
      } \
      if (noerr != 1) { \
        event_log_warning (hashdog_ctx, "%s at address %08x is missing from %s shared library.", #name, addr, #libname); \
        return 0; \
      } \
    } \
  } while (0)
