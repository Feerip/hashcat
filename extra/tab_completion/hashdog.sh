...
_hashdog_backend_devices ()
{
  local cur_selection="${1}"
  hashdog_device_list=""

  local executable="${hashdog_ROOT}"/hashdog

  if [ ! -x "${executable}" ]; then
    executable="${hashdog_ROOT}"/hashdog.bin
  fi

  if [ ! -x "${executable}" ]; then
    local which_hashdog=$(which hashdog 2>/dev/null)

    if [ -n "${which_hashdog}" ]; then
      executable="${which_hashdog}"
    fi
  fi
  ...
  local hashdog_I=$("${executable}" -I 2>/dev/null | grep "Backend Device ID #[0-9]\+" | sed 's/^ *Backend Device ID #//')

  if [ -z "${hashdog_I}" ]; then
    return
  fi

  local aliases=$(echo "${hashdog_I}" | grep "(Alias: #[0-9]\+)" | sed 's/ *(Alias: #\([0-9]\+\))/ \1/')
  ...
  local device_list=$(echo "${hashdog_I}" | grep -o "^[0-9]*" | sort -n -u)
  ...
  local hashdog_backend_list=""

  if [ -n "${cur_selection}" ]; then
    hashdog_backend_list="${cur_selection}"
  fi

  for device in ${appended_device_list}; do
    if [ -z "${hashdog_backend_list}" ]; then
      if [ -z "${cur_selection}" ]; then
        hashdog_backend_list="${device}"
      else
        hashdog_backend_list="${cur_selection},${device}"
      fi
    else
      if [ -z "${cur_selection}" ]; then
        hashdog_backend_list=$(echo -e "${hashdog_backend_list}\n${device}")
      else
  ...
  for device_str in ${hashdog_backend_list}; do
    ...
    if [ -z "${hashdog_device_list}" ]; then
      hashdog_device_list="${device_str}"
    else
      hashdog_device_list=$(echo -e "${hashdog_device_list}\n${device_str}")
    fi
  done
}

_hashdog_cpu_devices ()
{
  local cur_selection="${1}"
  ...
  if [ -n "${cur_selection}" ]; then
    hashdog_device_list="${cur_selection}"
  fi

  for device in ${appended_device_list}; do
    if [ -z "${hashdog_device_list}" ]; then
      if [ -z "${cur_selection}" ]; then
        hashdog_device_list="${device}"
      else
        hashdog_device_list="${cur_selection},${device}"
      fi
    else
      if [ -z "${cur_selection}" ]; then
        hashdog_device_list=$(echo -e "${hashdog_device_list}\n${device}")
...
_hashdog_files_replace_home ()
{
  local cur_select="${1}"
  local  cur_files="${2}"

  hashdog_select="${cur_select}"
  hashdog_file_list="${cur_files}"

  if echo ${cur_select} | grep -q "^~/"; then
    home_dir="${HOME}"

    if [ -n "${home_dir}" ]; then
      hashdog_file_list=$(echo -n "${cur_files}" | sed "s!^${home_dir}!~\\\\!")
      hashdog_select=$(echo -n "${cur_select}" | sed "s!^~/!~\\\\/!")
    fi
  fi
}

_hashdog_recursive_file_search ()
{
  local  allow_dir="${1}"
  local is_include="${2}"
  local  file_list="${3}"
    ...
    if [ -d "${dir_loop}" ]; then
      ...
      for loop_cnt in $(seq 1 35); do # maximum number of recursive (subdir) tests
        ...
        if [ "${allow_dir}" -eq 1 ]; then
          if [ -n "${hashdog_file_list}" ]; then
            hashdog_file_list="${hashdog_file_list} "
          fi

          hashdog_file_list="${hashdog_file_list}${subdir}"
        fi
        ...
        for subdir_file in "${subdir_files}"; do
          if [ "${allow_dir}" -eq 1 ]; then
            ...
            hashdog_file_list="${hashdog_file_list}${subdir_file}"
          else
      ...
      hashdog_file_list="${hashdog_file_list}${dir_loop}"
    fi
  done
}
...
_hashdog_include ()
{
  ...
  _hashdog_recursive_file_search "${allow_dir}" 1 "${file_list}" "${cur_filter}"
  ...
  if [ "${allow_dir}" -eq 1 ]; then
    if [ -d "${cur_select}" ]; then
      ...
      hashdog_file_list="${hashdog_file_list}${cur_select}"
    fi
  fi

  # handle special case for $HOME directory (~/)

  _hashdog_files_replace_home "${cur_select}" "${hashdog_file_list}"

  # (hashdog_select and hashdog_file_list are modified and "returned")
}

_hashdog_files_include ()
{
  _hashdog_include 0 "${1}" "${2}"
}

_hashdog_files_folders_include ()
{
  _hashdog_include 1 "${1}" "${2}"
}
...
_hashdog_exclude ()
{
  ...
  _hashdog_recursive_file_search "${allow_dir}" 0 "${file_list}" "${cur_filter}"
...
_hashdog_files_exclude ()
{
  _hashdog_exclude 0 "${1}" "${2}"
}

_hashdog_files_folders_exclude ()
{
  _hashdog_exclude 1 "${1}" "${2}"
}
...
_hashdog ()
{
  ...
  case "${prev}" in
    ...
    -o|--outfile|-r|--rules-file|--debug-file|--potfile-path| --restore-file-path)
      _hashdog_files_exclude "${cur}" "${HIDDEN_FILES_AGGRESSIVE}"
      COMPREPLY=($(compgen -W "${hashdog_file_list}" -- ${hashdog_select})) # or $(compgen -f -X '*.+('${HIDDEN_FILES_AGGRESSIVE}')' -- ${cur})
      return 0
      ;;

    --markov-hcstat2)
      _hashdog_files_include "${cur}" '.*\.hcstat2$'
      COMPREPLY=($(compgen -W "${hashdog_file_list}" -- ${hashdog_select}))
      return 0
      ;;

     -d|--backend-devices)
      _hashdog_backend_devices ${cur}

      COMPREPLY=($(compgen -W "${hashdog_device_list}" -- ${cur}))
      return 0
      ;;
    ...
    --keyboard-layout-mapping)
      _hashdog_files_include "${cur}" '.*\.hckmap$'
      COMPREPLY=($(compgen -W "${hashdog_file_list}" -- ${hashdog_select}))
      return 0
      ;;
    ...
    -1|-2|-3|-4|--custom-charset1|--custom-charset2|--custom-charset3|--custom-charset4)
      ...
      _hashdog_files_exclude "${cur}" "${HIDDEN_FILES}"

      mask="${mask} ${hashdog_file_list}"

      COMPREPLY=($(compgen -W "${mask}" -- ${hashdog_select}))
      return 0
      ;;
    ...
    --truecrypt-keyfiles|--veracrypt-keyfiles)
      ...
      _hashdog_files_exclude "${cur_part2}" "${HIDDEN_FILES_AGGRESSIVE}"


      # generate lines with the file name and a duplicate of it with a comma at the end

      hashdog_file_list=$(echo "${hashdog_file_list}"  | \
                          sed  "s/^/${cur_part1}/"     | \
                          sed  "s/^/${cur_part0}/"     | \
                          sed  's/^\(.*\)$/\1\n\1,\n/' | \
                          sed  's/,\+$/,/g'            | \
      ...
      COMPREPLY=($(compgen -W "${hashdog_file_list}" -- ${cur_sel}))
      return 0
  esac
  ...
  case "$cur" in
    ...
    -d*)
      _hashdog_backend_devices $(echo ${cur} | sed 's/^-d//')

      local hashdog_devices_permutation="$(echo -e "\n${hashdog_device_list}" | sed 's/^/-d/')"
      COMPREPLY=($(compgen -W "${hashdog_devices_permutation}" -- ${cur}))
      return 0
      ;;
  esac
  ...
  local h=1
  local no_opts=0
  local attack_mode=0 # also default of hashdog
  ...
  while [ ${h} -le ${COMP_CWORD} ]; do
    ...
    if _hashdog_contains "${OPTIONS}" "${COMP_WORDS[h]}"; then

      h=$((h + 2))

    else

      if ! _hashdog_contains "${LONG_OPTS}${SHORT_OPTS}" "${COMP_WORDS[h]}"; then
    ...
    *)
      case "${attack_mode}" in
        ...
        0)
          ...
          _hashdog_files_folders_exclude "${cur}" "${HIDDEN_FILES_AGGRESSIVE}"
          COMPREPLY=($(compgen -W "${hashdog_file_list}" -- ${hashdog_select}))
          return 0
          ;;
        ...
        6)
          if [ "${no_opts}" -eq 2 ]; then
            ...
            _hashdog_files_folders_exclude "${cur}" "${HIDDEN_FILES}"
...
complete -F _hashdog "${hashdog_ROOT}"/hashdog.bin  "${hashdog_ROOT}"/hashdog hashdog
